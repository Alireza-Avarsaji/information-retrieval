...
class NormalizeHandler:
    ...
    def base_normalizer(self):
       ...
       for row in openpyxl.load_workbook(self.file).active.iter_rows():
           curr_doc_token_list = self.stop_words(self.remove_punctuation(row[0].value))
           for pos, token in enumerate\
                       (curr_doc_token_list):
               if token in self.pos_index:
                   self.pos_index[token][0] = self.pos_index[token][0] + 1
                   if index in self.pos_index[token][1]:
                       self.pos_index[token][1][index].append(pos)
